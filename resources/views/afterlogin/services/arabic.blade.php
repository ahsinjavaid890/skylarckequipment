    ...
    <div style="direction: rtl;" class="container">
        <ul style="text-align: center;">
            <?php foreach(DB::table('services')->where('status' , 1)->get() as $r){ ?>
                <li class="<?php if($id == $r->id){ echo "activebottomnavbar"; } ?>"><a href="{{ url('servicedetails') }}/{{ $r->id }}">{{$r->arabicname}}</a></li>
            <?php }  ?>
        </ul>
    </div>
</div>
...
<div style="margin-top: 20px;margin-bottom: 20px;" class="container">
  <h2 style="text-align: right;">الإعلانات</h2>
    <div class="row">
        ...
        <div class="col-lg-4 col-md-6">
            <div class="single-blog-post bg-fffbf5">
                ...
                <div class="post-content">
                    ...
                    <h3 style="text-align: right;">{{$r->arabicname}}</h3>
                    <div style="text-align: right;">
                        {!! $r->arabicdescription !!}
                   </div>
                </div>
